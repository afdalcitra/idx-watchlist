...
export default async function handler(req, res) {
  ...
  try {
    ...
    const url = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=20y&events=dividends`;
    ...
    const ttmSum = dividends.filter(d => d.date >= ttmCutoff).reduce((s, d) => s + d.amount, 0);

    const years5 = [lastYear, thisYear - 2, thisYear - 3, thisYear - 4, thisYear - 5];
    const avg5 = years5.reduce((s, y) => s + (byYear[y] || 0), 0) / 5;

    const yieldCalc = (amount) => currentPrice ? (amount / currentPrice) * 100 : null;
